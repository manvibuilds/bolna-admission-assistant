      ...
      <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
            Complete history of all enquiry calls
          </p>
        </div>
        <button
          onClick={fetch}
      ...
      {loading ? (
        ...
        <div className="card p-16 text-center">
          ...
          <p className="text-sm text-zinc-400 mt-2">
            Calls will appear here once parents start calling the agent.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {logs.map((log, i) => (
            <div
              key={log.id}
              className="card p-5 flex items-start gap-4 hover:shadow-sm transition-shadow"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 flex-wrap">
                  <p className="font-medium text-zinc-900 dark:text-zinc-100">
                    {log.child_name || "Unknown Child"}
                  </p>
                  {log.grade && (
                    <span className="px-2 py-0.5 rounded-full text-xs bg-purple-100 text-purple-700 border border-purple-200 dark:bg-purple-900/20 dark:text-purple-300 dark:border-purple-800">
                      {log.grade}
                    </span>
                  )}
                ...
                {log.query_summary && (
                  <p className="text-xs text-zinc-600 dark:text-zinc-400 mt-1.5 bg-gray-50 dark:bg-zinc-800 rounded-lg px-3 py-1.5">
                    "{log.query_summary}"
                  </p>
                )}
                {log.callback_requested === "yes" && (
                  <p className="text-xs text-blue-600 dark:text-blue-400 mt-1.5 flex items-center gap-1">
                    📞 Callback requested{" "}
                    {log.callback_time ? `at ${log.callback_time}` : ""}
                  </p>
                )}
              </div>
              <div className="text-right shrink-0">
                ...
                <button
                  ...
                  className="mt-2 text-zinc-400 dark:text-zinc-500 hover:text-red-500 dark:hover:text-red-400 transition-colors"
                >
                  <Trash2 size={13} />
                </button>
              </div>
